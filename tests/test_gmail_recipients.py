...
def test_weekly_summary_list_requires_every_recipient_to_be_allowlisted():
    recipients, error = validate_weekly_summary_recipients(
        "first@example.com,missing@example.com",
        "first@example.com,second@example.com",
    )

    assert recipients == ["first@example.com", "missing@example.com"]
    assert error == "COLLEAGUE_WEEKLY_SUMMARY_TO contains a recipient not in GMAIL_ALLOWED_RECIPIENTS"


def test_weekly_summary_list_rejects_malformed_entries():
    recipients, error = validate_weekly_summary_recipients(
    ...
    assert error == "COLLEAGUE_WEEKLY_SUMMARY_TO contains an invalid email address"
