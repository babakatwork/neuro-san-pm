...
MAX_WEEKLY_SUMMARY_RECIPIENTS = 20
...
def validate_weekly_summary_recipients(
    weekly_value: str,
    allowed_value: str,
) -> tuple[list[str], str | None]:
    """Validate the complete weekly list before any recipient receives a message."""
    recipients = parse_email_list(weekly_value)
    if not recipients:
        return [], None
    if len(recipients) > MAX_WEEKLY_SUMMARY_RECIPIENTS:
        return recipients, (
            f"COLLEAGUE_WEEKLY_SUMMARY_TO exceeds the "
            f"{MAX_WEEKLY_SUMMARY_RECIPIENTS}-recipient safety limit"
        )
    if any(not is_email_address(recipient) for recipient in recipients):
        return recipients, "COLLEAGUE_WEEKLY_SUMMARY_TO contains an invalid email address"
    allowed = set(parse_email_list(allowed_value))
    if any(recipient not in allowed for recipient in recipients):
        return recipients, "COLLEAGUE_WEEKLY_SUMMARY_TO contains a recipient not in GMAIL_ALLOWED_RECIPIENTS"
